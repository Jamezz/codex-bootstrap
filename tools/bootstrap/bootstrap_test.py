...
class BootstrapSmokeTest(unittest.TestCase):
    ...
    def test_bootstrap_rewrites_java_template_checkout_into_standalone_project(self) -> None:
        with tempfile.TemporaryDirectory(prefix="codex-bootstrap-smoke-") as temp_dir:
            ...
            self.assertIn('<module name="Indentation">', checkstyle_config)
            self.assertIn('<property name="basicOffset" value="4"/>', checkstyle_config)
            self.assertIn('<property name="caseIndent" value="4"/>', checkstyle_config)
            self.assertIn('<property name="arrayInitIndent" value="4"/>', checkstyle_config)
            self.assertIn('<property name="lineWrappingIndentation" value="4"/>', checkstyle_config)
